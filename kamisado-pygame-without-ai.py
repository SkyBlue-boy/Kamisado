...
def msg_blocked():
    if turn == 0: # turn of blacks
        txt = "Blocked: next turn -> {} in 3 secs".format('white')
        _message_margin(txt, black, white)
    else:
        txt = "Blocked: next turn -> {} in 3 secs".format('black')
        _message_margin(txt, white, black)
    ...
    while running:
        ...
        if not win and tower_to_move >=0 and is_blocked():
            ...
            if tower_to_move_pos not in blocked_pos_list:
                # increase deadlock count for checking deadlock type 2
                blocked_pos_list.append(tower_to_move_pos)

                # switch turn and the tower to move in the next turn
